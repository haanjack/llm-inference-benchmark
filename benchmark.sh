...
if [[ "${mode}" == "test" ]]; then
    ...
    python3 run_vllm_benchmark.py --env-file ${env_file} --model-name ${model_dir} --vllm-image ${docker_image} --bench-scope ${mode} --gpu-devices ${gpu_ids} --custom-scope-file=configs/customs/custom_combination.txt
else
